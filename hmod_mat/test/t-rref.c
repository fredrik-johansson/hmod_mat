...
int check_rref_form(long * perm, hmod_mat_t A, long rank)
{
...
main(void)
{
    ...
    for (i = 0; i < 1000 * flint_test_multiplier(); i++)
    {
        ...
        hmod_mat_init(D, 2*m, n, mod);

        hmod_mat_randtest(A, state);
        hmod_mat_init_set(B, A);
        hmod_mat_init_set(C, A);

        rank1 = hmod_mat_rref(B);
        ...
        for (j = 0; j < m; j++)
        {
            ...
            for (k = 0; k < n; k++)
                hmod_mat_entry(D, perm[j], k) =
                    nmod_mul(hmod_mat_entry(A, j, k), c, A->mod);
        }
        ...
        for (j = 0; j < m; j++)
        {
            do { c = n_randint(state, mod); } while (c == 0);
            for (k = 0; k < n; k++)
                hmod_mat_entry(D, perm[m + j], k) =
                    nmod_mul(hmod_mat_entry(B, j, k), c, A->mod);
        }

        rank2 = hmod_mat_rref(D);
        equal = (rank1 == rank2);
        ...
        if (equal)
        {
            for (j = 0; j < rank2; j++)
                for (k = 0; k < n; k++)
                    equal = equal && (hmod_mat_entry(B, j, k) ==
                                        hmod_mat_entry(D, j, k));
            for (j = rank2; j < 2 * rank2; j++)
                for (k = 0; k < n; k++)
                    equal = equal && (hmod_mat_entry(D, j, k) == 0);
        }
        ...
        if (!equal)
        {
            ...
            hmod_mat_print_pretty(A); printf("\n\n");
            hmod_mat_print_pretty(B); printf("\n\n");
            hmod_mat_print_pretty(D); printf("\n\n");
            abort();
        }
