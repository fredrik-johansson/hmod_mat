...
hmod_mat_rref(hmod_mat_t A)
{
    ...
    P = _perm_init(hmod_mat_nrows(A));
    rank = hmod_mat_lu(P, A, 0);
    _perm_clear(P);
    ...
    hmod_mat_init(U, rank, rank, A->mod.n);
    hmod_mat_init(V, rank, n - rank, A->mod.n);
    ...
    {
        while (hmod_mat_entry(A, i, j) == 0UL)
        {
            nonpivots[k] = j;
            k++;
            j++;
    ...
    {
        for (j = 0; j <= i; j++)
            hmod_mat_entry(U, j, i) = hmod_mat_entry(A, j, pivots[i]);
    }

    for (i = 0; i < n - rank; i++)
    {
        for (j = 0; j < rank; j++)
            hmod_mat_entry(V, j, i) = hmod_mat_entry(A, j, nonpivots[i]);
    }

    hmod_mat_solve_triu(V, U, V, 0);

    /* Clear pivot columns */
    for (i = 0; i < rank; i++)
    {
        for (j = 0; j <= i; j++)
            hmod_mat_entry(A, j, pivots[i]) = (i == j);
    }
    ...
    {
        for (j = 0; j < rank; j++)
            hmod_mat_entry(A, j, nonpivots[i]) = hmod_mat_entry(V, j, i);
    }
